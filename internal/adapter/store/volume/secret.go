...
func (s *VolumeStore) DeleteSecret(secretName string) error {
	volumeName := fmt.Sprintf("%s%s", SecretVolumePrefix, secretName)
...
func (s *VolumeStore) GetSecret(secretName string) (*core.Secret, error) {
	volumeName := fmt.Sprintf("%s%s", SecretVolumePrefix, secretName)
...
func (s *VolumeStore) GetSecrets(selector labels.Selector) (core.SecretList, error) {
...
func (s *VolumeStore) StoreSecret(secret *corev1.Secret) error {
	volumeName := fmt.Sprintf("%s%s", SecretVolumePrefix, secret.Name)
